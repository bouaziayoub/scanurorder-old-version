...
// Register using Api
import { RegisterApi } from "../../../api/exportApi";
// Import Elemets
import {
  PasswordInput,
  Input,
    ...
    try {
      ...
      if (name === "") {
        ...
        if (!errorValidationPassorwd) {
          const data = await RegisterApi(name, surName, email, password);
          ...
          if (data.token) {
            ...
            setError(t("errors.requestRegister"));
          }
        }
      }
    } catch (error) {
      ...
      {!authorized ? (
        <Form>
          <div className="mb-md-1 mt-md-1">
            <h4 className=" mb-4 text-uppercase">{t("register.h4")}</h4>
